...
-- Basic policies so admins can manage roles later (optional minimal policy)
-- Only admins can select/insert/update/delete roles; until admins exist, only service role can manage via SQL.
...
-- 4) Add explicit admin-only SELECT policy on internship_registrations
